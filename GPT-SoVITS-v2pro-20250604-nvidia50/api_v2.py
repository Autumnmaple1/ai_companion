...
async def tts_handle(req: dict):
    ...
    if streaming_mode == 0:
        ...
        return JSONResponse(
            status_code=400,
            content={
                "message": f"the value of streaming_mode must be 0, 1, 2, 3(int) or true/false(bool)"
            },
        )
    ...
    try:
        # 将耗时的模型初始化运行放入线程池，防止阻塞事件循环
        tts_generator = await asyncio.to_thread(tts_pipeline.run, req)
        ...
        if streaming_mode:
            ...
            # 将耗时的模型推理迭代放入线程池
            sr, audio_data = await asyncio.to_thread(next, tts_generator)
